...
void solve()
{
    ll i,j,a,b,c,m,x,p,n;
  
    ll k,d,w;
   cin>>n;
   ll arr[3+1];
    
    for(i=1;i<=3;i++)cin>>arr[i];

    // bool one=false, two= false, three= false;
    // for(i=1;i<=n;i++)
    //     if(arr[i]==1)one=true;
    //     else if(arr[i]==2)two=true;
    //     else if(arr[i]==3)three=true;
    // ll key=0;
    // if(!one)key=1;
    // else if(!two)key=2;
    // else key=3;
    if((arr[2]==1 && arr[3]==2)||(arr[3]==1 && arr[1]==2))cout<<"YES"<<endl;
    else if((arr[2]==1 && arr[1]==3)||(arr[2]==3 && arr[3]==1))cout<<"YES"<<endl;
    else if((arr[1]==2 && arr[2]==3)||(arr[1]==3 && arr[3]==2))cout<<"YES"<<endl;
    else cout<<"NO"<<endl;
