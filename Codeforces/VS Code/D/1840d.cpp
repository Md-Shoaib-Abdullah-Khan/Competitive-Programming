...
void solve()
{
    ...
    ll ans=INF,cnt=0,siz=ceil((double)(arr[n-1]-arr[0])/3);
    cnt=arr[0]+siz;
    
    for(i=0;i<n-2;i++){
        a=binary_search(arr, i+1, n-1, arr[i+1]+(arr[n-1]-arr[i+1])/2);
        b=ceil((double)(arr[a-1]-arr[i+1])/2);
        c=ceil((double)(arr[n-1]-arr[a])/2);
        ll temp = max(b,c);
        b=ceil((double)(arr[i]-arr[0])/2);
        temp= max(temp, b);
        
       
        ans=min(ans,temp );
        //cout<<arr[i]<<" "<<arr[a-1]<<" "<<arr[a]<<" "<<temp<<endl;
    }
